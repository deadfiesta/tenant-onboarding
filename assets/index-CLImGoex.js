...
`;function f(s,i,l){let o=0;document.querySelectorAll(s).forEach(e=>{e.addEventListener("animationend",()=>{o++,o===i&&l()})})}document.addEventListener("DOMContentLoaded",()=>{c()});function c(){d.innerHTML=p;const s=document.getElementById("line");s&&document.documentElement.style.setProperty("--total-length",s.getTotalLength()),f("#ascenda-logo",9,g)}function g(){d.innerHTML=h,v();const s=document.querySelector("button");s&&s.addEventListener("click",c)}function v(){const s=document.querySelectorAll(".absolute");function i(l){const{clientX:o,clientY:e}=l,t=window.innerWidth/2,n=window.innerHeight/2;s.forEach(r=>{const a=r.dataset.speed||10,u=-(o-t)/a,C=-(e-n)/a;r.style.transform=`translate(${u}px, ${C}px)`})}document.removeEventListener("mousemove",i),document.addEventListener("mousemove",i)}
